...
function useGetFinancial() {
  const financialid = localStorage.getItem('financialsheetid')
  const ownerid = localStorage.getItem('uuid')
  return useQuery({
      queryKey: ['financialValues', financialid, ownerid],
      queryFn: async () => {
      const response = await fetch(`https://3.89.162.4:8000/sheetapi/getValues?id=${financialid}&uuid=${ownerid}`);
      if (!response.ok) {
...
const FinancialSheet = () => {
  const { data, isError, isLoading } = useGetFinancial();
